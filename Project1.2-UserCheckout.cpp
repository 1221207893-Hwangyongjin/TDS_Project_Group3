...
public:
    ...
    void displayOrders() {
        ...
        for (int i = topstack; i >= 0; --i) {
            Order& order = reinterpret_cast<Order&>(stack[i]);
    ...
    // Save the order to a file (different format than Product)
    void saveOrdersToFile(const string& filename) {
        ofstream file(filename);
        for (int i = 0; i <= topstack; ++i) {
            Order& order = reinterpret_cast<Order&>(stack[i]); // type casting
            file << order.orderId    << ","
                 << order.username   << ","
                 << order.productId  << ","
                 << order.productName << ","
...
private:
	...
	ADTstack orderHistory;
	
	// Generate unique order number (example: timestamp + random number)
    string generateOrderId() {
        time_t now = time(0);
        return to_string(now) + "_" + to_string(rand() % 1000);
    }	
...
public:
	...
	void checkout() {
	    while (!cart.empty()) {
	        ...
	        // Generate order
	        Order order;
	        order.orderId = generateOrderId(); // Generate a unique order ID
    ...
    void saveOrdersToFile() {
        string filename = "user_" + username + "_orders.txt";
        orderHistory.saveOrdersToFile(filename);
    }

    // load order history from file 
    void loadOrdersFromFile() {
        string filename = "user_" + username + "_orders.txt";
        orderHistory.loadOrdersFromFile(filename);
    }	
    ...
    do {
        ...
        case 3: {
		    ...
		    if (confirm == 'y' || confirm == 'Y') {
		        ...
		        currentUser->saveOrdersToFile(); // save order history to file
